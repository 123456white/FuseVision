...
- 最外层 widget 分两栏：左侧边栏、模型管理主体，比例 **1:5**（可通过分割器拖拽调整）。
- 左侧边栏分上下两栏：
  - **左上栏**（固定高度约 90px）：新建模型、打开模型两个按钮（左图标右文字），图标使用 `NewProject.png`/`OpenProject.png`。
  - **左下栏**（自适应）：当前模型详细信息区
    - 标题"当前模型"
    - 详细信息内容（模型文件路径、绑定的数据集、标签类别、实例数、创建/修改时间）
- **右侧主体**（纯卡片画廊）：
  - 网格布局展示所有模型卡片
  - 底部固定横向步骤引导条
...
- 固定尺寸 200x210px，圆角边框风格
- 卡片内容：
  - 图标区（180x160px，显示项目图标）
  - 底部信息区：模型名称 + 任务类型
- 交互：
  - **单击**：选中卡片，更新左侧详情和步骤引导条状态
  - **双击**：打开模型并跳转到数据集管理模块
- 选中/悬停状态：边框颜色变化区分

#### 底部步骤引导条

- 横向均匀分布 6 个步骤：项目管理 → 模型管理 → 数据集管理 → 数据标注 → 数据拆分 → 模型训练
- 当前步骤高亮，已完成步骤显示绿色对勾 ✓，未完成步骤显示灰色数字
- 顶部分割线与主体区域分隔
- 步骤 0（项目管理）自动标记为已完成
...
- **布局**：垂直方向分三部分：
  ...
  **第一栏：任务类型选择**
  - 左栏：4 个可选任务类型（单选按钮组）
    ...
    - 物体检测(自由矩形)
  - 右栏：当前选中类型的详细描述（8行左右）
  - 明暗主题适配：图标资源 `TaskSegmentation.png`/`_D.png` 等

  **第二栏：模型信息**
  - 模型名称输入框（必填，提示"仅允许英文字母、数字和下划线"）
  - 文件路径输入框 + 浏览按钮（默认定位到 `dl_models/项目名/`，扩展名 `.fvdl`）
  - 模型描述输入框（可选）
...
- **创建逻辑**：
  1. 调用 `ModelConfig::createModel()` 生成 `.fvdl` 文件
  2. 在 `dl_datasets/项目名/` 下创建 `模型名_dataset/` 数据集目录
  3. 生成初始 `dataset_config.json`（空类别）
  4. 更新 `.fvproj` 文件，添加模型记录
  5. 刷新卡片画廊，自动选中新建的模型
...
- **选定模型时**（从 `.fvdl` 和项目状态读取）：
  ...
  - 绑定的数据集：`dl_datasets/项目名/模型名_dataset/`
  - 标签类别：`xx 个类别`
  - 实例个数：`xxx 图，xxx 实例`
  - 创建时间 / 修改时间：`yyyy-mm-dd hh:mm`

### 4.4 打开模型

- 文件选择器选择 `.fvdl` 文件，默认定位到 `dl_models/项目名/`
- 校验模型是否属于当前项目（`project_ref` 匹配）
- 校验失败提示"项目不匹配"
- 成功后更新当前模型 ID，同步卡片选中状态，刷新详情和步骤引导条

### 4.5 状态同步

- **双击卡片进入数据集管理**：发送 `openDatasetRequested(modelId)` 信号，跳转 Tab 2
- **信号链**：
  - `modelCreated(name)` → 日志输出"模型已创建: xxx"
  - `modelOpened(name)` → 日志输出"模型已打开: xxx"
  - `currentModelChanged(name)` → 日志输出"当前模型切换至: xxx"
  - `openDatasetRequested(modelId)` → 日志输出"跳转数据集管理: xxx"
